...
<head>
    ...
    <title>TradePort - EA Converter & Hosting</title>
                ...
                <div className="min-h-screen p-6">
                    ...
                    <header className="max-w-6xl mx-auto flex items-center justify-between py-6">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-white/10 to-white/6 flex items-center justify-center border border-white/10 shadow-md">
                            ...
                            <div>
                                <h1 className="text-2xl font-semibold tracking-wide">TradePort</h1>
                                <p className="text-sm text-white/70">EA Host · Converter · Mobile-ready dashboard</p>
                            </div>
                        </div>
                        <nav className="flex items-center gap-4">
                            <button className="px-4 py-2 rounded-md bg-white/6 border border-white/10 hover:bg-white/8">Docs</button>
                            <button className="px-4 py-2 rounded-md bg-white/6 border border-white/10 hover:bg-white/8">Support</button>
                    ...
                    <main className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
                        {/* Left Panel - Upload */}
                        <section className="md:col-span-1 glass-card rounded-2xl p-5 shadow-lg">
                            <h2 className="text-lg font-semibold mb-3">Upload EA</h2>
                            <p className="text-sm text-white/70 mb-4">
                                Drag your .mq5 / .ex5 / .mq4 file here — this prototype simulates conversion & hosting.
                            </p>
                            ...
                            <div className="mt-4 flex gap-2">
                                <button onClick={mockConvert} className="flex-1 rounded-md py-2 bg-gradient-to-r from-white/6 to-white/4 text-black font-semibold">
                                    Convert (mock)
                                </button>
                                <button onClick={toggleHosting} className="rounded-md py-2 px-4 bg-white/6 border border-white/10">
                                    {hosting ? 'Stop' : 'Host'}
                                </button>
                            </div>
                            ...
                            <div className="mt-4 text-xs text-white/60">
                                <strong>Note:</strong> This is a frontend prototype. Actual EA hosting requires a backend service.
                            </div>
                        </section>

                        {/* Right Panel - Dashboard */}
                        <section className="md:col-span-2 glass-card rounded-2xl p-6 shadow-lg">
                            <div className="flex items-start justify-between">
                                <div>
                                    <h2 className="text-lg font-semibold">Dashboard</h2>
                                    <p className="text-sm text-white/70">Live status & logs — mobile preview at the bottom.</p>
                                </div>
                                <div className="text-sm text-white/60">
                                    User: <span className="font-medium">demo@tradeport.local</span>
                                </div>
                            </div>
                            ...
                            <div className="mt-6">
                                ...
                                <div className="h-40 overflow-y-auto rounded-md bg-black/30 p-3 text-xs border border-white/6">
                                    {logs.length === 0 && <div className="text-white/60">No logs yet — actions will appear here.</div>}
                                    {logs.map((l, i) => (
                            ...
                            <div className="mt-6">
                                <h3 className="text-sm font-semibold mb-2">Mobile Preview</h3>
                                <div className="w-full max-w-xs mx-auto rounded-2xl bg-neutral-900 border border-white/6 p-4 shadow-md">
                                    <div className="bg-gradient-to-b from-neutral-800 to-neutral-900 rounded-lg p-3">
                                        <div className="flex items-center justify-between">
                                            <div className="text-sm font-medium">TradePort Mobile</div>
                                            <div className="text-xs">{hosting ? 'Online' : 'Offline'}</div>
                                        </div>
                                        <div className="mt-3 text-xs text-white/60">EA: {eaFile ? eaFile.name : '—'}</div>
                                        <div className="mt-2 flex gap-2">
                                            <button className="flex-1 rounded-md py-1 text-xs bg-white/6">Start</button>
                                            <button className="flex-1 rounded-md py-1 text-xs bg-white/6">Stop</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <footer className="max-w-6xl mx-auto mt-8 text-center text-xs text-white/60">
                        <div className="mb-2">
                            Prototype only — does not run EAs. For production you'll need a backend service.
                        </div>
                        <div className="inline-block text-left bg-white/4 border border-white/6 p-4 rounded-lg">
                            <div className="font-medium">Next steps (suggested):</div>
                            <ol className="mt-2 text-xs text-white/70 list-decimal list-inside">
                                <li>Create a Node.js + Express backend with file-upload endpoint</li>
                                <li>Use Docker to wrap EA runtime on free cloud services</li>
                                <li>Create mobile app with React Native or Ionic</li>
                                <li>Deploy using free hosting providers</li>
                            </ol>
                        </div>
                    </footer>
                </div>
